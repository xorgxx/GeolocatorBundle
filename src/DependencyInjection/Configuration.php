        ...
        $rootNode
            ->children()
                ...
                ->booleanNode('provider_fallback_mode')->defaultFalse()->info('Active le mode de secours avec provider local si aucun provider externe n\'est disponible')->end()
                        ...
                        ->booleanNode('messenger_enabled')->defaultFalse()->end()
                        ->booleanNode('rabbit_enabled')->defaultFalse()->end()
                        ->booleanNode('redis_messenger_enabled')->defaultFalse()->end()
                        ->booleanNode('mercure_enabled')->defaultFalse()->end()
                        ...
                        ->arrayNode('list')
                            ...
                            ->arrayPrototype()
                                ->children()
                                    ->booleanNode('enabled')
                                        ->defaultTrue()
                                        ->info('Active ou désactive ce fournisseur')
                                    ->end()
                                    ->scalarNode('dsn')
                                        ->isRequired()
                ...
                // Configuration pour le mode asynchrone
                ->arrayNode('async')
                    ->addDefaultsIfNotSet()
                    ->children()
                        ->booleanNode('enabled')
                            ->defaultFalse()
                            ->info('Active le mode asynchrone pour les requêtes de géolocalisation')
                        ->end()
                        ->scalarNode('transport')
                            ->defaultValue('async')
                            ->info('Nom du transport Messenger à utiliser pour les messages asynchrones')
                        ->end()
                        ->integerNode('retry_limit')
                            ->defaultValue(3)
                            ->info('Nombre maximal de tentatives pour les messages en échec')
                            ->min(0)
                            ->max(10)
                        ->end()
                        ->integerNode('retry_delay')
                            ->defaultValue(1000)
                            ->info('Délai en millisecondes entre les tentatives')
                        ->end()
                    ->end()
                ->end()
